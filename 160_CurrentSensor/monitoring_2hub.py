...
IP = '192.168.1.200'                           # Ethernet connection (fastest)
# IP = '192.168.2.1'                          # SCU200's WiFi network
PORT = 502                                    # Depends on the port configured on the Web UI
SLAVE_IDS = range(1, 161)                     # IDs of all Modbus slave devices
...
def poll_slaves_multithread(stop_event, all_poll_data):                                         # Each thread polls a range of slave IDs
    ...
    while not stop_event.is_set():
        ...
        # Divide polling among 5 threads, one for each flat cable/CMS BUS port
        ranges = [(1, 32), (33, 64), (65, 96), (97, 128), (129, 160)]
...
def periodic_polling_loop(stop_event, all_poll_data):
    while not stop_event.is_set():
        ...
        try:
            ...
            save_all_data_to_excel(all_poll_data, filename='backup_poll_data.xlsx')
        except Exception as e:
...
st.title("160 Current Sensors Setup")
