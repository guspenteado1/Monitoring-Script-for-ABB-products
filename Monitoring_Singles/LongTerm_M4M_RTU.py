...
SLAVE_IDS = [2, 9, 11]                        # IDs of all Modbus slave devices
POLL_INTERVAL = 60                            # Time between polls in seconds
...
SLAVE_DEVICE_ASSIGNMENTS = [
    {"slaves": SLAVE_IDS, "device_type_key": "M4M"}
]
...
# Helper function to get device configuration for a given slave ID
...
def read_slave_with_client(slave_id, thread_id, client):
    ...
    for var_info in variables:
        ...
        try:
            ...
            # Step 1: Apply specific decoding if mapped
            ...
            # Step 2: Apply scale factor if defined AND value is numeric (int or float)
            if name in scale_factors and isinstance(current_value, (int, float)):
                current_value *= scale_factors[name]
            
            # Step 3: Ensure the final value is a scalar or None. Flatten any remaining lists for Excel
...
def poll_slaves_multithread(stop_event, all_poll_data):
    ...
    while not stop_event.is_set():
        ...
        slave_ids = [2, 9, 11]  # IDs específicos desejados
...
st.sidebar.write(f"**Port:** {PORT_SERIAL}")
